...
const host = "";
 ...
 const DataService = {
	...
	addCoalitionParty :  async (coalition) =>{
		 				 
	    fetch(host+"data/coalition/"+coalition+"/party",  {method: 'POST'})
	        .then(response => response.json());
			    			
	},
	editCoalitionParty :  async (id,party) =>{
		 
	    fetch(host+"data/coalition/party/"+id,  
  		{method: 'PUT',
  		 headers: { 'Content-Type': 'application/json' },
  		 body:JSON.stringify(party)})
      .then(response => response.json());
